...
class Luban private constructor(builder: Builder) : Handler.Callback {
    ...
    private fun compressReal(context: Context, path: InputStreamProvider): File {
        ...
        result = if (mCompressionPredicate != null) {
            ...
            if (Checker.SINGLE.needCompress(mLeastCompressSize, source)) Engine(path,
                outFile, focusAlpha).compress() else File(source)
        }
        return result
    }
    ...
    override fun handleMessage(msg: Message): Boolean {
        when (msg.what) {
            ...
            MSG_COMPRESS_ERROR -> {
                ...
                if (mNewCompressListener != null) {
                    mNewCompressListener.onError(msg.data.getString(KEY_SOURCE),
